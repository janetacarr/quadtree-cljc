...
(defn split
  ...
  [{:keys [nodes level bounds max-objects max-levels objects] :as quadtree}]
  (when (and nodes level bounds
             max-objects max-levels objects
             (:x bounds) (:y bounds)
...
(defn get-quadrant
  ...
  (try
    ...
    (catch NullPointerException npe
           (throw (ex-info "Caught NullPointException: Did you forget a keyword?"
                           {:params #{quadtree bounds-obj}})))))
